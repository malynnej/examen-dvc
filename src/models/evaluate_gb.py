...
def evaluate(X_test: pd.DataFrame, y_test:pd.Series, y_train:pd.Series):
    ...
    model = joblib.load(models_dir/"model_gbr.pkl")
    ...
    print("Model GBR MSE:", mean_squared_error(y_test, y_pred))
    print("Model GBR R2:", r2_score(y_test, y_pred))

    results_dir= get_path("results.results_dir")
    results_dir.mkdir(parents=True, exist_ok=True)
    # save predictions
    preds = pd.DataFrame({"y_true": y_test, "y_pred": y_pred})
    preds.to_csv(results_dir / "predictions_gbr.csv", index=False)

    mse = mean_squared_error(y_test, y_pred)
    r2 = r2_score(y_test, y_pred)

    
    with open(results_dir / "scores_gbr.json", "w") as f:
